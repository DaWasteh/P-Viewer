...
- **HTML in Markdown (#6):** eingebettetes HTML wird wie auf GitHub dargestellt statt verworfen – `<img>` mit Breite/Höhe, `<a>`, `<hr>`, `<h1>`–`<h6>`, `<b>`/`<strong>`, `<i>`/`<em>`, `<br>`, Listen, vollständige Tabellen mit `width`, `align`, `colspan`/`rowspan`, `<details>`/`<summary>`, `<kbd>`, `<sub>`/`<sup>` und `<picture>`; Markdown und HTML lassen sich beliebig mischen. Die Pipeline parst Roh-HTML mit `rehype-raw` und sanitisiert danach unverändert mit GitHubs Allowlist: Skripte, `<style>`, Frames, Formulare, Event-Handler, Inline-Styles sowie `javascript:`- und `data:`-URLs werden entfernt, IDs und Namen aus Roh-HTML erhalten gegen DOM-Clobbering das Präfix `user-content-` (Sprungmarken wie `[Installation](#install)` zu `<a name="install">` funktionieren trotzdem)
- Relative HTML-Bilder (`<img src="bilder/a.png">`, `./…`) werden identisch zu Markdown-Bildern aus dem Dokumentordner geladen – mit derselben Sicherheitsgrenze: Pfade, die per `../` aus dem Dokumentordner herausführen, bleiben für beide gesperrt; relative `<picture>`-Quellen fallen auf das aufgelöste `<img>` zurück. Externe Bilder werden über HTTPS geladen (App-CSP `img-src … https:`), unverschlüsseltes HTTP bleibt blockiert
- HTML-Überschriften erscheinen in der Gliederung und lassen sich falten; die Gliederung wird jetzt aus dem gerenderten Dokument erzeugt, sodass ihre Sprungziele immer mit den IDs der Vorschau übereinstimmen
- Tabellenausrichtung korrigiert: eine CSS-Regel überstimmte bisher `align` – betraf `<td align="center">` und auch die GFM-Spaltenausrichtung (`:-:`, `-:`)
- **„Öffnen mit“ und Kontextmenü unter Windows (#7):** Der Installer schrieb seit v0.1.0 alle Öffnen-Befehle (ProgIDs und `Applications\p-viewer.exe`) mit einem ungültigen NSIS-Escape als `$"…\p-viewer.exe$" $"%1$"` in die Registry. Windows konnte P-Viewer darüber nicht starten, blendete es deshalb in „Öffnen mit“ teils aus, und eine Neuinstallation überschrieb sogar den korrekten Befehl, den Windows beim manuellen Auswählen der EXE angelegt hatte – danach öffnete ein Doppelklick die Datei nicht mehr. Die Befehle lauten jetzt korrekt `"…\p-viewer.exe" "%1"`; jede Installation und jedes Update repariert bestehende Einträge
- Neuer Explorer-Eintrag **Mit P-Viewer öffnen** für alle 306 unterstützten Endungen (nicht für `*`, unter Windows 11 in „Weitere Optionen anzeigen“)
- Updates und Neuinstallationen über eine bestehende Installation lassen ProgIDs, „Öffnen mit“ und Kontextmenü stehen und schreiben sie nur neu, damit Windows eine gewählte Standard-App nicht verwirft; die echte Deinstallation entfernt weiterhin ausschließlich P-Viewers Einträge und lässt fremde Werte, Standardzuordnungen und UserChoice unangetastet. Wirksam ab Updates von v0.2.1 aus: beim manuellen Upgrade von v0.2.0 oder älter läuft noch der alte Deinstaller, dort muss eine verlorene Standard-App einmalig neu gewählt werden
- Die Installer-Hooks werden aus `associations.json` generiert (`scripts/nsis-hooks.mjs`) und selbst geprüft: `npm run check:associations` verlangt für jede Endung genau eine ProgID mit Öffnen-Befehl, SupportedTypes-, OpenWithProgids- und Kontextmenü-Eintrag, symmetrische Deinstallation und keine Standard-/UserChoice-Schreibzugriffe; NSIS-Warnung 6000 (unbekannte Variable) ist in den Hooks ein Build-Fehler. Neuer Windows-CI-Job `npm run test:installer-hooks` kompiliert die Hooks mit `makensis` und prüft Installation, erneute Installation, Update, Drüberinstallieren und Deinstallation direkt in der Registry
- Regressionstests: Roh-HTML-Rendering und XSS-Fälle, Bildpfade, Tabellen, gemischte Dokumente, README-Struktur, Gliederung, CSP, Installer-Hooks sowie ein Browser-Test für Ausrichtung, entschärfte Event-Handler, `<details>`, Gliederung und Sprungmarken
