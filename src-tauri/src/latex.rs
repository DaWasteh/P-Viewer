...
#[tauri::command]
pub fn detect_latex_engines() -> Vec<LatexEngineInfo> {
    ENGINES
        .iter()
        .map(|engine| {
            let version = executable_version(engine.executable);
            LatexEngineInfo {
...
fn select_engine(requested: &str) -> Option<EngineSpec> {
    if requested == "auto" {
        return ENGINES
            .iter()
            .copied()
            .find(|engine| executable_version(engine.executable).is_some());
    }

    ENGINES
        .iter()
        .copied()
        .find(|engine| engine.id == requested && executable_version(engine.executable).is_some())
}
